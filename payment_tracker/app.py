...
def add_member():
    try:
        ...
        new_member = Member(
            ...
            amount_due=amount,
            original_amount=amount,  # Store the original amount
...
def reset_total(type):
    try:
        if type == 'collected':
            # Just return 0 for total collected without modifying database
            return jsonify({
                'success': True, 
                'total_collected': 0
            })
        elif type == 'pending':
            # Just return 0 for pending amount without modifying database
            return jsonify({
                'success': True, 
                'pending_amount': 0
            })
