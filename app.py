...
class Payment(db.Model):
    ...
    original_amount_due = db.Column(db.Float, nullable=False)
    remarks = db.Column(db.Text, nullable=True)  # Allow null by default
...
def update_payment():
    try:
        ...
        new_payment = Payment(
            ...
            original_amount_due=member.amount_due,
            remarks=remarks
        )
        db.session.add(new_payment)
        member.amount_due = member.amount_due - float(data['amount_paid'])
        db.session.commit()
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})
...
def reports():
    ...
    ).filter(func.date(Payment.payment_date) == today).scalar()

    # Sum all pending amounts (from index view) without date filtering.
    pending_payments = db.session.query(
        func.coalesce(func.sum(Member.amount_due), 0)
    ).scalar()

    total_collected = total_collected if total_collected > 0 else 0
    pending_payments = pending_payments if pending_payments > 0 else 0

    all_members = Member.query.all()

    recent_transactions = db.session.query(Payment, Member)\
        .join(Member, Payment.member_id == Member.id)\
        .order_by(Payment.payment_date.desc()).limit(10).all()

    transactions_data = []
    for payment, member in recent_transactions:
        transactions_data.append({
            'id': payment.id,
            'date': payment.payment_date.strftime('%d-%m-%Y'),
            'member': member.name,
            'amount': payment.amount_paid,
            'status': 'Paid' if payment.amount_paid > 0 else 'Pending'
        })
    ...
    return render_template("reports.html",
                           ...
                           revenue_labels=[], revenue_data=[], status_data=[])
...
def get_member_details(member_id):
    ...
    payment_history = [{
        ...
        'remarks': payment.remarks if payment.remarks else "No remarks"
...
def update_transaction(transaction_id):
    try:
        ...
        if member:
            ...
            member.amount_due = member.amount_due - payment_difference
